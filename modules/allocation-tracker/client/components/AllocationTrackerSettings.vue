          ...
          <div class="space-y-3">
            <label class="block">
              ...
              <textarea
                v-model="bulkJql"
                rows="3"
                placeholder='project in (AIPCC, RHOAIENG) AND type in (Story, Bug, Spike) AND "Activity Type" is EMPTY AND created >= -30d'
...
import { getTeams, saveTeams, saveProjects, discoverBoards, getProjects, classifyIssue, bulkClassifyIssues, getClassificationConfig, saveClassificationConfig } from '../services/api.js'
...
const bulkJql = ref('project in (AIPCC, RHOAIENG, INFERENG, RHAIENG) AND type in (Story, Bug, Spike, Task, Epic) AND "Activity Type" is EMPTY AND created >= -30d')
